...
export default function EditTherapistProfileModal({
  profile,
  onClose,
  onSave,
  onDelete,
  ...
  onClose: () => void;
  onSave: (next: AdminTherapistProfile) => void;
  onDelete?: () => void;
}) {
  const [draft, setDraft] = useState<AdminTherapistProfile>(profile);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Edit therapist profile"
      onMouseDown={(e) => {
    ...
    >
      <div className="flex w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-mgmt-surface-container-lowest shadow-2xl max-h-[calc(100vh-2rem)]">
        <div className="flex items-center justify-between border-b border-mgmt-surface-container px-5 py-4 sm:px-8 sm:py-6">
          <h2 className="text-xl font-bold tracking-tight text-mgmt-on-surface">Edit profile</h2>
          <button
            type="button"
        ...
        <div className="flex flex-col gap-3 bg-mgmt-surface-container-low px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-8 sm:py-6">
          <div>
            {onDelete ? (
              <button
                type="button"
                className="rounded-xl bg-red-600 px-5 py-2.5 text-sm font-bold text-white transition-opacity hover:opacity-90 active:scale-95"
                onClick={onDelete}
              >
                Delete
              </button>
